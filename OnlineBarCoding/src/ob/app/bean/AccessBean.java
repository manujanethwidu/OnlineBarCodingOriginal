package ob.app.bean;

import java.util.Date;

public class AccessBean {
	String config;
	String tyresize;
	String tyrerim;
	String tyretype;
	String brand;
	String sidewall;
	String barcode;
	String qualitygrade;
	String remarks;
	Date dateofmanufacture;
	String tyrestatus;
	Date CreateDate;
	String processIDnw;
	String processIDOld;
	int Status;
	String stencilno;
	String DispatchStatus;
	int MergeStatus;
	public String getConfig() {
		return config;
	}
	public void setConfig(String config) {
		this.config = config;
	}
	public String getTyresize() {
		return tyresize;
	}
	public void setTyresize(String tyresize) {
		this.tyresize = tyresize;
	}
	public String getTyrerim() {
		return tyrerim;
	}
	public void setTyrerim(String tyrerim) {
		this.tyrerim = tyrerim;
	}
	public String getTyretype() {
		return tyretype;
	}
	public void setTyretype(String tyretype) {
		this.tyretype = tyretype;
	}
	public String getBrand() {
		return brand;
	}
	public void setBrand(String brand) {
		this.brand = brand;
	}
	public String getSidewall() {
		return sidewall;
	}
	public void setSidewall(String sidewall) {
		this.sidewall = sidewall;
	}
	public String getBarcode() {
		return barcode;
	}
	public void setBarcode(String barcode) {
		this.barcode = barcode;
	}
	public String getQualitygrade() {
		return qualitygrade;
	}
	public void setQualitygrade(String qualitygrade) {
		this.qualitygrade = qualitygrade;
	}
	public String getRemarks() {
		return remarks;
	}
	public void setRemarks(String remarks) {
		this.remarks = remarks;
	}
	public Date getDateofmanufacture() {
		return dateofmanufacture;
	}
	public void setDateofmanufacture(Date dateofmanufacture) {
		this.dateofmanufacture = dateofmanufacture;
	}
	public String getTyrestatus() {
		return tyrestatus;
	}
	public void setTyrestatus(String tyrestatus) {
		this.tyrestatus = tyrestatus;
	}
	public Date getCreateDate() {
		return CreateDate;
	}
	public void setCreateDate(Date createDate) {
		CreateDate = createDate;
	}
	public String getProcessIDnw() {
		return processIDnw;
	}
	public void setProcessIDnw(String processIDnw) {
		this.processIDnw = processIDnw;
	}
	public String getProcessIDOld() {
		return processIDOld;
	}
	public void setProcessIDOld(String processIDOld) {
		this.processIDOld = processIDOld;
	}
	public int getStatus() {
		return Status;
	}
	public void setStatus(int status) {
		Status = status;
	}
	public String getStencilno() {
		return stencilno;
	}
	public void setStencilno(String stencilno) {
		this.stencilno = stencilno;
	}
	public String getDispatchStatus() {
		return DispatchStatus;
	}
	public void setDispatchStatus(String dispatchStatus) {
		DispatchStatus = dispatchStatus;
	}
	public int getMergeStatus() {
		return MergeStatus;
	}
	public void setMergeStatus(int mergeStatus) {
		MergeStatus = mergeStatus;
	}

	

}

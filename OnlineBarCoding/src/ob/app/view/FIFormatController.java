...
public class FIFormatController implements Initializable {
	...
	////////////
	BasicDatafromTireCodeBean basicDatafromTireCodeBean = new BasicDatafromTireCodeBean();
	FIReportBean fiReportBean = new FIReportBean();
	private MainApp mainApp;
	// if no change-NO
	// PID Change by firstTime-CHGFI
	// PID Change by FG CHGRBC
	String pidChangeType = "-";
	// for update REBCHistory Tbl
	boolean blnPidCnge = false;
	int originalPid = 0;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		tblPid.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, nw) -> {
			BasicDatafromTireCodeBean bdata = new BasicDatafromTireCodeBean();
			bdata.setSizebasic(nw.getSb());
			bdata.setLugtype(nw.getLt());
			bdata.setConfig(nw.getCon());
			bdata.setRimsize(nw.getRs());
			bdata.setTiretype(nw.getTt());
			bdata.setSwmsg(nw.getSwmsg());
			bdata.setBrand(nw.getBr());
			bdata.setMoldNo("???");
			bdata.setSn(basicDatafromTireCodeBean.getSn());
			bdata.setPid(nw.getPid());
			bdata.setTireCode(0);
			pidChangeType = "PID";
			blnPidCnge = true;
			setBean(bdata);
			// blnPidCnge for update REBCHistory Tbl

		});
	}

	public void setSN(int snx) {
		basicDatafromTireCodeBean.setSn(snx);
	}

	public void setBean(BasicDatafromTireCodeBean bean) {
		this.basicDatafromTireCodeBean = bean;
		...
		lblSN.setText(Integer.toString(basicDatafromTireCodeBean.getSn()));
		// First Time attempt without changing selected pid
		if (!blnPidCnge) {
			originalPid = bean.getPid();
	...
	private void ShowBER() {
		// Show BER Scene
		mainApp.addBER(basicDatafromTireCodeBean);
	}
	...
	private void enterButton() throws SQLException {
		...
		try (Connection conn = createConn.GetConn()) {
			...
			PidAccessTbl pidAccessFile = new PidAccessTbl();
			BCPendingTbl bcPendingTbl = new BCPendingTbl();

			// Cross check pid availability in access file
			boolean blnPIDinAccessFile = pidAccessFile.pidAvaialbilityCheck(conn, basicDatafromTireCodeBean.getPid());
			if (blnPIDinAccessFile) {

				// Cross check already barceded by fireport table has sn.
				boolean blnFfireportavl = fiReportTbl.serialNoAvailablityCheck(conn, basicDatafromTireCodeBean.getSn());
				if (!blnFfireportavl) {

					boolean blnInsertedDefectTbl = fiReportTbl.InsertFIReportTbl(conn, basicDatafromTireCodeBean,
							fiReportBean);
					boolean blnUpdatedStkTbl = stkTbl.updateStkTblREBC(conn, basicDatafromTireCodeBean, fiReportBean,
							"A");
					boolean blnBCPendingTbl = bcPendingTbl.insertBCPendingTbl(conn, basicDatafromTireCodeBean, "A",
							pidChangeType,originalPid);
					boolean blnInsertReBCHistoryTbl = true;
					// Insert new entry to rebchistory Tbl in case of PID Change
					if (blnPidCnge) {
						blnInsertReBCHistoryTbl = false;
						REBCHistoryTbl rb = new REBCHistoryTbl();
						blnInsertReBCHistoryTbl = rb.insertHistory(conn, originalPid, basicDatafromTireCodeBean, "A",
								"A");
						if (!blnInsertedDefectTbl) {// Error reporting
							JOptionPane.showMessageDialog(null,
									"FIFormatController.EnterButton insertReBCHistory error ");
						}
					}
					// Cross check again
					blnInsertedDefectTbl = fiReportTbl.CrossChleckInsert(conn, basicDatafromTireCodeBean.getSn());
					blnUpdatedStkTbl = stkTbl.CrossChleckUpdate(conn, basicDatafromTireCodeBean.getSn());
					blnBCPendingTbl = bcPendingTbl.CrossChleckInsert(conn, basicDatafromTireCodeBean.getSn());

					if (blnInsertedDefectTbl && blnUpdatedStkTbl && blnBCPendingTbl && blnInsertReBCHistoryTbl) {
						// Go again SNlist
						// Reload the SNList
						conn.commit();

						BarCodeCreator barCodeCreator = new BarCodeCreator();
						String barCode = barCodeCreator.create(basicDatafromTireCodeBean, "A");
					...
					JOptionPane.showMessageDialog(null,
							basicDatafromTireCodeBean.getSn()
									+ " is  already avaialble in QUALITY REPORT! Can  not BarCode ",
							"Allready BarCoded Tire", 1);
				}
			} else {// (blnPIDinAccessFile)
				JOptionPane.showMessageDialog(null,
						basicDatafromTireCodeBean.getPid() + " is not avaialble in Access File! Can  not BarCode ",

...
public class JoinTbl {
	...
	public BasicDatafromTireCodeBean getTireDetailFrmstkTblc(Connection conn, int sn) throws SQLException {

		BasicDatafromTireCodeBean bean = new BasicDatafromTireCodeBean();
		String sql = " select tiresizebasic,config,lugtypecap,rimsize,tiretypecap,swmsg,brand,p.pid pid_a, stktbl.sn sn_a, "
				+ " stktbl.mfgdate mfgdate_a   " 
				+ " from stock.stk stktbl "
				+ " join srtspec.pid p on p.pid= stktbl.pid " 
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql);) {
			...
			if (rs.next()) {
				...
				JOptionPane.showMessageDialog(null, "JoinTbl.getTireDetail() SN is not avialable 1");
			}
	...
	public ObservableList<PidFxBean> getAllPIDs(Connection conn) throws SQLException {
		...
		String sql = " select p.pid pid,sb.sizebasicid sbid,sb.tiresizebasic sb,lt.lugtypeid ltid,lt.lugtype lt,con.configid conid,con.config con,rs.rimsizeid rsid,rs.rimsize rs,"
				+ "tt.tiretypeid ttid,tt.tiretypecap tt,sw.swmsgid swmsgid,sw.swmsg swmsg,br.brandid brid,br.brand br  from srtspec.pid p "
				+ "join srtspec.size s on s.sizeid = p.sizeid "
				+ "join srtspec.sizebasic sb on sb.sizebasicid = s.sizebasicid "
